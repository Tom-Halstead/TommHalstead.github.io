...
// if (window.link.pathname.endsWith("story.html")) {
...
/**
 * Utility function to normalize paths (handles subdirectories).
 */
const getPageName = () =>
  window.location.pathname.split("/").pop() || "index.html";

/**
 * Inserts dynamic HTML for the header and footer on specific pages.
 */
const insertHTML = function () {
  const page = getPageName();
  const pagesRequiringInsert = ["story.html", "work.html", "contact.html"];

  // Skip insertion if on index.html (already has header/footer)
  if (!pagesRequiringInsert.includes(page)) return;
  ...
  if (header) header.insertAdjacentHTML("afterbegin", headerMarkup);
  if (footer) footer.insertAdjacentHTML("afterbegin", footerMarkup);
...
const navHighlights = function () {
  ...
  document.querySelectorAll(".nav-btn").forEach((link) => {
    ...
    const isHomePage = page === "index.html" && href === "index.html";

    if (isHomePage || page === href) {
      link.setAttribute("aria-current", "page");
    }
  });
};
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (getPageName() === "story.html") {
    const gallery = document.querySelector("section");
    if (gallery) gallery.addEventListener("click", modalWindow);
  }
