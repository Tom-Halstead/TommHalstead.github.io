...
/* -------------------- TYPEWRITER (no flash, caret shown first) -------------------- */
...
function typewriter(el, speedMs = 35, startDelayMs = 1000) {
  ...
  if (!el.querySelector(".t-char")) wrapChars(el);

  // Insert caret BEFORE first char and show it immediately
  ...
  const step = () => {
    ...
    chars[i].insertAdjacentElement("afterend", caret); // move caret along
    i++;
